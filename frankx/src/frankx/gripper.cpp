...
bool Gripper::move(double width) {  // [m]
  if (width - width_calibration <= 0.) {
    width = width_calibration;
  }
  try {
    ...
    if (current_width > 0.01 && std::abs(current_width - width) > 0.01) {
      has_error = true;
      std::cerr << "Gripper does (" << current_width << ") not do what it should (" << width << +")." << std::endl;
      return false;
    }
    if (result) {
      has_error = false;
...
bool Gripper::open() { return move(max_width); }
