...
struct MotionGenerator {
  ...
  static std::tuple<std::array<double, 7>, std::array<double, 7>, std::array<double, 7>> getInputLimits(RobotType* robot,
  ...
  static std::tuple<std::array<double, 7>, std::array<double, 7>, std::array<double, 7>> getInputLimits(RobotType* robot,
    ...
    if (waypoint.max_dynamics || data.max_dynamics) {
      auto max_velocity =
          MotionGenerator::VectorCartRotElbow(0.8 * translation_factor * robot->max_translation_velocity,
                                              0.8 * robot->max_rotation_velocity, 0.8 * robot->max_elbow_velocity);
      auto max_acceleration = MotionGenerator::VectorCartRotElbow(
          ...
          0.8 * derivative_factor * robot->max_rotation_acceleration, 0.8 * derivative_factor * robot->max_elbow_acceleration);
  ...
  static void setInputLimits(ruckig::InputParameter<7>& input_parameters, RobotType* robot, const Waypoint& waypoint,
                             const MotionData& data) {
    std::tie(input_parameters.max_velocity, input_parameters.max_acceleration, input_parameters.max_jerk) =
        getInputLimits(robot, waypoint, data);
