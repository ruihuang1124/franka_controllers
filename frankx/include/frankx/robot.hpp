...
class Robot : public franka::Robot {
  ...
  const KinematicChain<7> kinematics = KinematicChain<7>({{{0.0, 0.333, 0.0},
                                                           {-M_PI / 2, 0.0, 0.0},
                                                           {M_PI / 2, 0.316, 0.0},
                                                           {M_PI / 2, 0.0, 0.0825},
                                                           {-M_PI / 2, 0.384, -0.0825},
 ...
 public:
  ...
  //! Whether the robots try to continue an interrupted motion due to a libfranka position/velocity/acceleration discontinuity
  ...
  void setBehavior(double major_torque_limit, double minor_torque_limit, double force_limit);
  ...
  bool move(ImpedanceMotion& i_motion, JointMotion p_motion);
  bool move(ImpedanceMotion& i_motion, JointVelocityMotion v_motion);
  bool move(JointMotion p_motion, ImpedanceMotion& i_motion, MotionData& data);
  bool move(JointVelocityMotion v_motion, ImpedanceMotion& i_motion, MotionData& data);

  bool move(JointVelocityMotion motion);
  bool move(JointVelocityMotion motion, MotionData& data);
