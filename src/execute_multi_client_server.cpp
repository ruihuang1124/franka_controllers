...
public:
    MultiClientCallServer() {
        ...
        execute_curi_joint_server_ = nh_.advertiseService("/execute_curi_joint_position",
...
//    left_arm_msg_->request.speed_ratio
    ...
    torso_msg_->request.speed_ratio = req.speed_ratio;
    ...
    for (int i = 0; i < num_of_torso_joints_; ++i) {
        torso_msg_->request.goal_state.position[i] = req.goal_state.position[i];
    }
    for (int i = 0; i < num_of_arm_joints_; ++i) {
        left_arm_msg_->request.goal_state.position[i] = req.goal_state.position[i+num_of_torso_joints_];
        right_arm_msg_->request.goal_state.position[i] = req.goal_state.position[i+num_of_torso_joints_+num_of_arm_joints_];
    }
