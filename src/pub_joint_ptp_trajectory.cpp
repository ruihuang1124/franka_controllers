...
public:
    multiThreadService()
    {
        ...
        getCurrentJointStatesClientLeft = nh.serviceClient<roport::GetGroupJointStates>("/panda_left/get_group_joint_states");
        getCurrentJointStatesClientRight = nh.serviceClient<roport::GetGroupJointStates>("/panda_right/get_group_joint_states");
        ros::service::waitForService("/panda_left/get_group_joint_states");
        ros::service::waitForService("/panda_right/get_group_joint_states");
    }
...
private:
    ...
    ros::ServiceClient getCurrentJointStatesClientLeft, getCurrentJointStatesClientRight;
};

bool multiThreadService::pandaLeftCB(franka_controllers::createTrajectory::Request &req,
                                     franka_controllers::createTrajectory::Response &res) {
    roport::GetGroupJointStates get_current_joint;
    get_current_joint.request.result_type = "franka_panda_left";
    getCurrentJointStatesClientLeft.call(get_current_joint);
    if (get_current_joint.response.result_status){
        for (int i = 0; i < 7; ++i) {
            left_initial_joint_[i] = get_current_joint.response.joint_states[i];
        }
        ...
        ROS_WARN("Fail to get the current joint states!!!");
    }
...
bool multiThreadService::pandaRightCB(franka_controllers::createTrajectory::Request &req,
                                      franka_controllers::createTrajectory::Response &res) {
    roport::GetGroupJointStates get_current_joint;
    get_current_joint.request.result_type = "franka_panda_right";
    getCurrentJointStatesClientRight.call(get_current_joint);
    if (get_current_joint.response.result_status){
        for (int i = 0; i < 7; ++i) {
            right_initial_joint_[i] = get_current_joint.response.joint_states[i];
        }
        if (strcmp(req.targetPoint.c_str(), "home_point") == 0){
...
{
    ros::init(argc, argv, "Dual_Arm_PtP_Trajectory_Generator");
